...
class PuppetTheatre(wx.Frame):
	def __init__(self, parent, title, size):
		...
		loadAudio = fileMenu.Append(wx.ID_ANY, 'Load Audio', 'Load Audio from File')
		...
		self.Bind(wx.EVT_MENU, self.OnLoadAudio, loadAudio)
	...
	def OnLoadAudio(self, event):
		with wx.FileDialog(self, "Load Audio File", wildcard="Audio files (*.mp3)|*.mp3",
			...
			print(pathname)
			try:
				self.gamePanel.loadAudio(pathname)
			except IOError:
